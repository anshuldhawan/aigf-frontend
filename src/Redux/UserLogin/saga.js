...
function* forgotPasswordSaga({ payload, callBack }) {
  ...
  try {
    if (response?.data?.status === 200) {
      callBack && callBack(response?.data);
      yield put(ACTION.forgotPassword_Success(response.data?.data));
    } else {
      const error = response?.response?.data?.message;
      toast.error(error);
      yield put(ACTION.forgotPassword_Fail(response?.data?.error));
    }
  } catch (error) {
    yield put(ACTION.forgotPassword_Fail(error));
  }
}
